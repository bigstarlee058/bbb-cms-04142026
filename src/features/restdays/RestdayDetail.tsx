...
import { fetchRestday } from './api';
import { fetchEquipmentTitles } from '../workouts/api';
import Vimeo from '@u-wave/react-vimeo';

export const RestdayDetail = () => {
  const { restdayId } = useParams();
  const { addNotification } = useNotificationStore();
  const navigate = useNavigate();
  const { data, isLoading } = useQuery(['get-restday', restdayId], () => fetchRestday(restdayId), {
    onError: (err: ErrorMessage) => {
      addNotification({
        type: 'success',
        title: err.message,
      });
      navigate('/app/restdays');
    },
  });
